...
{/* Original Animated NFC Signal Waves Concept */}
export const NfcSignalWaves = ({ className = "w-12 h-12 text-[#6C4CFF]" }) => (
  <OriginalNfcSignal className={className} />
);

/**
 * Floating Hero Card (Requirement 2 & Requirement 8)
 * Continuous floating animation: Y (0 -> -15px -> 0), Rotation (0 -> 1.5° -> 0 -> -1.5° -> 0)
 * Duration: 5s, Ease: easeInOut, Repeat: infinite
 */
export const FloatingHeroCard = ({ name = "JOHN DOE", title = "Founder & CEO", company = "aikulb TECH" }) => (
  <Card3DTilt maxRotateX={4} maxRotateY={6}>
    <motion.div
      ...
      className="relative w-80 h-48 md:w-96 md:h-56 rounded-3xl p-6 glass-panel border border-[#6C4CFF]/30 shadow-[0_25px_60px_rgba(108,76,255,0.25)] flex flex-col justify-between overflow-hidden group hover:border-[#6C4CFF] transition-colors duration-500 select-none"
    >
      {/* Dynamic Ambient Violet Glow Background */}
      <div className="absolute -top-24 -left-24 w-48 h-48 bg-[#6C4CFF]/20 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700 pointer-events-none" />
      <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-[#3B82F6]/20 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700 pointer-events-none" />
...
export const BlackMetalCardVisual = ({ name = "JOHN DOE", title = "FOUNDER & CEO" }) => (
  <Card3DTilt maxRotateX={3} maxRotateY={5}>
    <div className="relative w-full aspect-[1.586] rounded-2xl p-5 bg-gradient-to-br from-[#1A1A1A] via-[#111111] to-[#090909] border border-neutral-700/60 shadow-2xl flex flex-col justify-between overflow-hidden select-none">
...
export const GoldMetalCardVisual = ({ name = "EXECUTIVE MEMBER" }) => (
  <Card3DTilt maxRotateX={3} maxRotateY={5}>
    <div className="relative w-full aspect-[1.586] rounded-2xl p-5 bg-gradient-to-br from-[#3D2C0D] via-[#241A06] to-[#120D03] border border-amber-500/50 shadow-2xl flex flex-col justify-between overflow-hidden select-none">
...
export const SmartStandVisual = () => (
  <div className="relative w-full aspect-square rounded-3xl p-6 bg-gradient-to-b from-[#111111] via-[#090909] to-black border border-[#6C4CFF]/30 shadow-2xl flex flex-col items-center justify-between select-none">
...
/**
 * Hand Tapping Card Phone Visual (Requirement 2 & Requirement 9)
 * Card holding animation over phone with expanding NFC signal waves.
 */
export const HandTappingCardPhoneVisual = () => (
  <div className="relative w-full max-w-lg mx-auto flex items-center justify-center py-6 select-none">
    ...
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-[#6C4CFF]/20 rounded-full blur-3xl pointer-events-none" />
    ...
    <div className="relative w-64 sm:w-72 h-[480px] bg-[#0A0A0A] rounded-[42px] border-4 border-[#222222] p-3 shadow-[0_25px_60px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col justify-between">
      ...
      <div className="w-full h-full bg-[#121212] rounded-[32px] pt-8 px-4 pb-4 overflow-hidden flex flex-col justify-between text-white relative z-10">
        ...
        <div className="bg-[#1A1A1A] border border-neutral-800 rounded-2xl p-4 text-center space-y-3 relative overflow-hidden shadow-lg">
          <div className="w-16 h-16 rounded-full mx-auto bg-gradient-to-tr from-[#6C4CFF] to-[#3B82F6] p-0.5 shadow-md">
          ...
          {/* Brand Badge */}
          <div className="inline-flex items-center space-x-1.5 bg-[#6C4CFF]/20 text-[#8B5CF6] border border-[#6C4CFF]/30 px-3 py-1 rounded-full text-[10px] font-bold font-manrope">
            <NfcIcon className="w-3 h-3 text-[#8B5CF6]" />
            <span>aikulb Verified</span>
          </div>

          {/* Action Buttons inside Phone Screen */}
          <div className="grid grid-cols-2 gap-2 pt-1">
            <button className="w-full py-2 rounded-xl aikulb-gradient-bg text-white font-bold text-xs font-manrope shadow-md shadow-[#6C4CFF]/30 hover:scale-[1.02] transition-transform">
              Save Contact
            </button>
            <button className="w-full py-2 rounded-xl bg-[#262626] hover:bg-[#333] text-neutral-200 font-bold text-xs font-manrope transition-colors">
              Share
            </button>
          </div>
        </div>
      ...
      {/* Tapping NFC Signal Wave Concept (Requirement 9) */}
